					  ...
					  mp_int * mp_XA, mp_int * mp_YA)
{
    ...
    if(lenParamXA<32){
        ...
        memset(tmp1,0x00,32);
        memcpy(tmp1+(32-lenParamXA),uzParam_XA,lenParamXA);
        memcpy(uzParam_XA,tmp1,32);
        lenParamXA = 32;
    }
    if(lenParamYA<32){
        unsigned char tmp1[32] ;
        memset(tmp1,0x00,32);
        memcpy(tmp1+(32-lenParamYA),uzParam_YA,lenParamYA);
        memcpy(uzParam_YA,tmp1,32);
        lenParamYA = 32;
    }
...
// 	memset(&signature, 0x00, sizeof(ECCSIGNATUREBLOB));
    ...
    memcpy(signedData+(32-ulTmp), signature, ulTmp);
    
    memset(signature,0x00,sizeof(signature));
    ret = Mp_Int2Byte(signature, &ulTmp, &mp_s);
    CHECK_RET(ret);
    if (ulTmp > 32)
    {
        ret = ERR_UNKNOWN;
    ...
    memcpy(signedData+32+(32-ulTmp), signature, ulTmp);
